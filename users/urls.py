...
from users.views import (CustomPasswordResetCompleteView, CustomPasswordResetConfirmView, CustomPasswordResetDoneView,
                         CustomPasswordResetView, UserDeleteView, UserListView, UserRegisterView, UserUpdateView,
                         email_verification)
...
urlpatterns = [
    ...
    path(
        "logout/",
        LogoutView.as_view(template_name="users/user_logout.html", next_page="/newsletter/home_page"),
        name="user_logout",
    ),
    ...
    path(
        "password-reset/<uidb64>/<str:token>/", CustomPasswordResetConfirmView.as_view(), name="password_reset_confirm"
    ),
